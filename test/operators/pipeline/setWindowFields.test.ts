...
describe(testPath(__filename), () => {
  ...
  describe("Numeric Range Window", () => {
    it("should use numeric range bounds with sortBy on a numeric field", () => {
      const numericData = [
        { _id: 1, score: 10, val: 1 },
        { _id: 2, score: 20, val: 2 },
        { _id: 3, score: 30, val: 3 },
        { _id: 4, score: 40, val: 4 },
        { _id: 5, score: 50, val: 5 }
      ];
      const result = aggregate(
        numericData,
        [
      ...
      // For score=10: range [0,20] -> vals 1,2 -> sum=3
      expect(result[0].nearbySum).toBe(3);
      // For score=30: range [20,40] -> vals 2,3,4 -> sum=9
      expect(result[2].nearbySum).toBe(9);
    });

    it("should handle range window with 'current' as begin", () => {
      const numericData = [
        { _id: 1, pos: 10, v: 1 },
        { _id: 2, pos: 20, v: 2 },
        { _id: 3, pos: 30, v: 3 }
      ...
      // For pos=10: range [10,20] -> vals 1,2 -> sum=3
      expect(result[0].sumFromCurrent).toBe(3);
      // For pos=30: range [30,40] -> val 3 -> sum=3
      expect(result[2].sumFromCurrent).toBe(3);
    });

    it("should handle numeric range with unbounded end", () => {
      const numericData = [
        { _id: 1, pos: 10, v: 1 },
        { _id: 2, pos: 20, v: 2 },
        { _id: 3, pos: 30, v: 3 }
      ...
      // For pos=10: range [5, +Inf] -> vals 1,2,3 -> sum=6
      expect(result[0].sumToEnd).toBe(6);
      // For pos=30: range [25, +Inf] -> only val 3 qualifies -> sum=3
      expect(result[2].sumToEnd).toBe(3);
    });
  });
});
